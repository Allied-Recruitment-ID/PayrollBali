...
Public Class classPayrollBali
    ...
    Public Function getDataTimeSheetPayrollBali(ByVal startDate As String, ByVal endDate As String, ByVal empName As String) As DataTable
        ...
        Try
            ...
            cmdmysql.Connection = dbConn.cnnMysql
            cmdmysql.CommandType = CommandType.Text
            cmdmysql.CommandText = "SELECT timesheetbali.id, timesheetbali.idImport, timesheetbali.lastName, timesheetbali.firstName, DATE_FORMAT(timesheetbali.dateTimeSheet, '%W, %d/%m/%Y') AS dateTimeSheet, timesheetbali.dateTimeSheet AS `dateTimeSheetSort`, timesheetbali.clockOn, timesheetbali.clockOff, timesheetbali.breaks, timesheetbali.actualHours AS 'actualHours', ROUND(timesheetbali.toBePaidHours, 2) AS 'toBePaidHours', ROUND(timesheetbali.baliBaseHourly, 2) AS 'baliBaseHourly', ROUND(timesheetbali.baliOvertime, 2) AS 'baliOvertime', ROUND(timesheetbali.baliHolidayPay, 2) AS 'baliHolidayPay', ROUND(timesheetbali.baliSickPay, 2) AS 'baliSickPay', ROUND(timesheetbali.baliFlexiTimeEarned, 2) AS 'baliFlexiTimeEarned', ROUND(timesheetbali.baliFlexiTimeTaken, 2) AS 'baliFlexiTimeTaken', ROUND(timesheetbali.baliOvertime15x, 2) AS 'baliOvertime15x', timesheetbali.created_at, timesheetbali.staff_add, timesheetbali.update_at, timesheetbali.staff_update, staffpayrollbali.cardId FROM timesheetbali LEFT JOIN staffpayrollbali ON CONCAT(staffpayrollbali.firstName,staffpayrollbali.lastName) = CONCAT(timesheetbali.firstName,timesheetbali.lastName) WHERE " & qry & " ORDER BY firstName, dateTimeSheetSort ASC"
            da.SelectCommand = cmdmysql
            da.Fill(dt)
            Return dt
